...
| **Markdown** | `.md` `.markdown` `.mdx` | ✅ Shipping | Rendered preview with an Edit/Preview toggle; links to local files are clickable. ` ```mermaid ` fences render as diagrams and `$…$`/`$$…$$` as math (KaTeX with `trust: false`, mermaid script-free). |
