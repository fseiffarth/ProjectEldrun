 ...
 *
 * The live bug (SimpleGNN, four tabs incl. three Claude conversations): the debounced
...
describe("persistScope — an empty layout only clears when it is meant", () => {
  ...
  it("refuses to vouch when the scope holds tabs that all get filtered out", async () => {
    // THE SimpleGNN CASE. The scope holds tabs, but every one of them belongs to a
