...
### Step 8 — Layout restructure: full-screen center with overlay panels

- [x] `app/panels/bottom_panel.py` created: `BottomPanel` with Root button, left/right panel toggles, scrollable project pills (active/warm indicators), + button (new/import popover)
- [x] `CenterPanel` is now the sole `Gtk.Overlay` child (`hexpand`, `vexpand`, `margin_bottom=40`)
- [x] `LeftPanel` and `RightPanel` are overlay children (`halign=START/END`); `_outer_paned` removed
- [x] `RightPanel` stripped: Root button, search field, + button removed; gear moved into PROJECTS header; `on_project_removed` callback added
- [x] `BottomPanel` pinned `valign=END`; project pills sync active/warm state; both panel toggles wired

### Desktop workspace management (Cinnamon)

- [x] `app/workspace_manager.py`: Cinnamon DBus `Eval` for mutations, Xlib EWMH for reads; in-memory `dict[project_id → workspace_idx]`
- [x] **Eldrun sticky**: `_NET_WM_STATE_STICKY` set on the Eldrun window at startup so it appears on all desktops
- [x] **Workspace assignment**: `_on_project_created` and startup restore both call `wm.allocate(id, name)` to append a named workspace per project
- [x] **Switch on activation**: `_on_pill_activate` calls `wm.activate(project_id)` to switch the active desktop
