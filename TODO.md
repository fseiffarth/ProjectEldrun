...
| J | [`todo/group-j-mail.md`](todo/group-j-mail.md) | Web & Mail Surfaces — URI routing to external apps (#33), plus in-app mail (#65) and browser (#61) counterparts, mail at-rest/OpenPGP encryption (#66), the IMAP session pool (#167), OAuth 2.0 / `XOAUTH2` (#168), and the mail task a local model would run (#202 — the 🧠 menu's **Mail** role tag ships, its consumer does not; **built out in Group Q**). **Mail security audit (#847–#858, 2026-09-17) — fixed the same day, untested live:** partial-signature spoofing, save-to-project symlink write (fence escape), decrypted attachments persisted, untimed IMAP stream drains, plus lower-severity credential/link/store findings. |
| Q | [`todo/group-q-mail-ai.md`](todo/group-q-mail-ai.md) | **Local-Model Mail Assistant (on-device, #203–#208) — implemented 2026-07-30, untested live** (toggles are per account behind `mail_ai_allow`). The consumer of J#202's Mail role tag: summarize a message, auto-file it Important/Urgent (distinct from keyword filters), formalize a reply from notes, and extract a calendar event or to-do card (review-before-create by default; full automation opt-in, off). The AI path is **loopback-only — stricter than `ollama_allow_remote_host`**; nothing about a message leaves the machine. Plan: [`docs/mail_local_ai_plan.md`](docs/mail_local_ai_plan.md). |
...
| N | [`todo/group-n-i18n.md`](todo/group-n-i18n.md) | Internationalization (i18n) — full app-wide translation coverage. **DONE** (6233 keys, 5-language parity, 2026-09-14). Plan/history: [`docs/i18n_translation_plan.md`](docs/i18n_translation_plan.md). |
