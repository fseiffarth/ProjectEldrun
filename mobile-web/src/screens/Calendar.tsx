...
export function Calendar() {
  ...
  return <main className="screen mobile-calendar-screen">
    ...
    {editing && <EventEditor editing={editing} calendars={data?.calendars ?? []} busy={busy} close={() => setEditing(null)} save={async (event) => { const eventId = editing.event?.id; const ok = await mutate(eventId ? { type: "update_event", event_id: eventId, event } : { type: "create_event", event }); if (ok) setEditing(null); }} remove={editing.event ? async () => { const eventId = editing.event?.id; if (eventId && window.confirm(`Delete “${editing.event?.title}”?`) && await mutate({ type: "delete_event", event_id: eventId })) setEditing(null); } : undefined} />}
    {manage && <CalendarManager calendars={data?.calendars ?? []} busy={busy} mutate={mutate} close={() => setManage(false)} />}
  </main>;
}
