...
| `RootReviewStrip.tsx` + `stores/rootReview.ts` | Also lists agent-written mail drafts (`stores/mail` `agentDrafts`) as rows that open the composer, never an Approve. Root-agent proposals: literal before/after rows, invisible-control stripping, outbound effects, digest-bound Approve/Reject/bulk approval and conditional Undo; count feeds the console and project header. |
