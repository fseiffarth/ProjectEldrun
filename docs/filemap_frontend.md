...
| `stores/agents/agentModels.ts` + `lib/agents/agentModel.ts` + `shared/agentSort.ts` | Model tag and ordering for Agents views (desktop + phone): model read from the agent's own transcript (`agent_tab_model`); Eldrun passes no model flag. |
