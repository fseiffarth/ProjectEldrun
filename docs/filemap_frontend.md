...
| `RootReviewStrip.tsx` + `stores/rootReview.ts` | Body of the proposals panel the console's ⚿ badge drops (store's `panel`), not a strip in the console body. Also lists agent-written mail drafts (`stores/mail` `agentDrafts`) as rows that open the composer, never an Approve. Root-agent proposals: literal before/after rows, invisible-control stripping, outbound effects, digest-bound ✓/✗ + bulk approval and conditional Undo; count feeds the console and project header, whose ⚿ button opens the console straight onto the panel. |
