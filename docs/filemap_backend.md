...
| `root_mcp.rs` | Root-console MCP tools (`docs/context/root_console.md`): revocable per-spawn token/grant map, policy-based staging, scoped reads, bounded git sweeps and RPC validation; separate self-targeted Scheduler identities and CLI wiring for `/mcp/schedule`. |
...
| `agent_fence.rs` | Default-on local agent fence: pure authority/root decisions, box union roots, fence-tool probe (bwrap / `sandbox-exec`), argv wrappers, working-root args. Remote/container/Windows report not-enforced. Fails closed; final private-state masks outrank project/allowlist mounts. |
...
| `mail_store.rs` | Encrypted mail persistence; folder search matches only subject/sender like IMAP, while other local lists may search previews. MCP draft changes compare the original row under the database lock, so composer ownership cannot be raced. |
