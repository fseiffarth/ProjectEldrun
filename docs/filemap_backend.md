...
| `root_mcp.rs` | The root console's MCP listener and Tauri-only review commands: loopback `POST /mcp`, `Origin` refused, per-tab bearer checked; applied rows emit `root-mcp-changed`, review changes emit a count. `root_mcp_status` carries neither port nor token, but does carry `WIRED_CLIS` for the CLI rows' read-only MCP chips. Design: `docs/context/root_console.md`. |
...
| `root_mcp.rs` | Root-console MCP tools (`docs/context/root_console.md`): per-spawn token → tab/caller map, annotation-based write staging, own-tab `proposals_list`, root-only CLI wiring. |
| `root_mcp_mail.rs` | The root MCP's nine mail tools (`docs/mail_mcp_plan.md`): `MailAccess` trait (impl: `commands::mail::AgentMail`), whole-result envelope, `strip_invisible`, `redact_urls`, caps, per-class draft isolation. Read tools answer `Caller::Reader` only. AppHandle-free. |
...
| `root_mcp_review.rs` | Backend-owned per-tab calendar copies and proposal log; content hashes, immutable rows/digests, atomic preconditions, conflict replay, conditional Undo and bounded decided history. AppHandle-free. |
