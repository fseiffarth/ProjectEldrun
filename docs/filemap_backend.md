...
| `agent_transcript.rs` | Agent transcript tail (6 MiB) for the phone's Focus view, as prompt/answer entries; resolved via `agent_session::read_agent_transcript` (Claude, Codex), `opencode_store` (OpenCode). |
...
| `opencode_store.rs` | OpenCode's session store (`opencode.db`), read-only: the newest top-level session of a tab's folder as Focus prompt/answer entries; queries only `session`/`message`/`part` (the file also holds account tokens). |
