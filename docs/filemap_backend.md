...
| `root_mcp_mail.rs` | The root MCP's nine mail tools (`docs/mail_mcp_plan.md`): `MailAccess` trait (impl: `commands::mail::AgentMail`), whole-result envelope, `strip_invisible`, `redact_urls`, caps, account grants and per-spawn draft isolation with atomic composer handoff. Read tools answer `Caller::Reader` only. AppHandle-free. |
