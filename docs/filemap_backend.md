...
| File | Purpose |
|------|---------|
| `projects.rs` | Project CRUD, scaffold/import, time-today (god module, #1). `find_project_conflict` is the one duplicate gate (#152), comparing a `ProjectSite` (local dir canonicalized; remote = SSH target + path). |
