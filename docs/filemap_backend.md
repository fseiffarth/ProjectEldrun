...
| `git_pull.rs` | Pull as fetch → preview → apply: `git_fetch` routes like push; the preview and the apply use the tracking ref (ff-only by default, merge only on the checked-out branch); a stopped merge is resolved via `git_merge_sides` (the `gitmerge` viewer), then committed or aborted. |
...
| `mail.rs` (search) | `mail_search` asks IMAP for subject/sender matches, backfills at most 1,000 headers, and reports when older matches were omitted; offline searches use the local index. |
...
| `print_native.rs` | `print_pdf_native`: a PDF's bytes → the OS print UI, the PDF itself to the printer (vector). Linux: GTK dialog + `GtkPrintJob` source file (hand-declared FFI). Windows: locked-down `print-*` WebView2 window + `ShowPrintUI` (Edge's PDFium). macOS: PDFKit `printOperation…` + `runOperation`. |
| `subwindow.rs` | Detached subwindow lifecycle (#42). `sync_detached_scope` (from `setScope` only) is the one visibility authority: hide/show elsewhere; native Wayland retires (asks the popout to settle unsaved work, then closes it; the `Destroyed` hook keeps the record) and falls back to minimize for unsaved work. `detach_subwindow` reserves its label under the registry lock. `detached_query` writes `?detached=<scope>&group=<gid>`. |
...
| `root_mcp.rs` | The root console's MCP listener and Tauri-only review commands: bounded loopback `POST /mcp`, authority/Origin checks before body reads, per-session grants/revocation and audit commands; applied rows emit `root-mcp-changed`, review changes emit a count. `root_mcp_status` carries neither port nor token, but does carry `WIRED_CLIS` for the CLI rows' read-only MCP chips. Also routes `/mcp/help` (`services::help_mcp`), `root_mcp_status.help`, and the window's `help_search`/`help_read`/`help_topics`. Design: `docs/context/root_console.md`. |
...
| `ide.rs` | "Open in <IDE>": `detect_project_ides` / `open_project_in_ide` / `set_ide_launcher`. Maps a project id to the scanned dir (a remote project → its local mirror), launches via `apps::do_launch` with `ORIGIN_PROJECT_IDE`; the program never comes from the tree. Detection is `services/ide_detect.rs`. |
...
| `mobile_control/` | AppHandle-free Eldrun Mobile sidecar core: loopback HTTP/PWA server, exact-origin device auth, bounded admin/desktop protocols, audit store, tmux PTY/WebSocket bridge. Raw project ids, paths, commands, tmux targets never cross the browser API; phone-facing schedule and prompt response types omit stored prefix commands, session ids and target ids. The phone writes tab labels, colours, order, close/create, schedules and prompts through the desktop bridge. Sessions live in memory only (sliding `auth::SESSION_IDLE`); every socket carries `limits::IDLE_TIMEOUT`; `pty_bridge` acks each input frame (`TerminalEvent::Ack`), replays history in ≤64 KB frames and sheds oldest output under flood. `discovery.rs`: scopes are projects, boxes and — behind `root_open`'s switch + review gate — the root console (`docs/mobile_root_plan.md`). `inbox.rs`: project inbox (`.eldrun/inbox/`) and the global one (`<state_dir>/inbox/`, `POST /api/v1/inbox`, listed/opened/deleted by leaf via `global_inbox_*`). |
...
| `agent_turn.rs` | Agent turn state from the agents' own hooks (`<live_sessions>/<ELDRUN_TAB_UID>.turn`): watches the live-sessions tree, maps uid → PTY, emits `agent-turn` (working/decision/done/idle) **plus a `job` flag** — whether a shell the agent BACKGROUNDED is running (Claude's `run_in_background` seam, or anything alive past its Stop; the tool call it waits on is not one), from one `/proc` walk for the whole fleet every 2 s. |
| `agent_session.rs` | Agent session resolvers (Claude/Codex/Vibe), hook installer (Claude/Codex events and Vibe `post_agent`), live-session records (`<uid>`, `.mode`, `.src`), Codex hook trust, `agent_session_model`. |
| `agent_transcript.rs` | Agent transcript tail (6 MiB) for the phone's Focus view, as prompt/answer entries plus an `agent` entry per spawned subagent carrying an opaque `subagent_token` (sha256 of the CLI's id) that reads that subagent's own conversation, resolved only among the tab session's subagents; resolved via `agent_session::read_agent_transcript` (Claude, Codex), `opencode_store` (OpenCode). |
| `codex_store.rs` | Codex's SQLite thread store (`~/.codex/state_<n>.sqlite`), read-only best-effort: `thread_model`, `thread_exists` (Codex stopped writing rollout files), `spawned_threads`/`descendant_threads` (subagents via `thread_spawn_edges`). |
| `opencode_store.rs` | OpenCode's session store (`opencode.db`), read-only: the newest top-level session of a tab's folder as Focus prompt/answer entries, child sessions (`parent_id`) as subagent entries; queries only `session`/`message`/`part` (the file also holds account tokens). |
...
| `root_mcp.rs` | Root-console MCP tools (`docs/context/root_console.md`): revocable per-spawn token/grant map, policy-based staging, scoped reads, bounded git sweeps and RPC validation; separate self-targeted Scheduler identities and CLI wiring for `/mcp/schedule`; the per-tab Helper lane and wiring for `/mcp/help`. |
| `schedule_mcp.rs` + `schedule_usage.rs` | Self-targeted schedule tools: strict prompt-only arguments, default-staged proposals, quotas, expiry, local recurrence/reset resolution and isolated RPC registry. AppHandle-free; `docs/context/agent_schedule_mcp.md`. |
| `help_mcp.rs` | Read-only `eldrun-help` MCP (`Caller::Helper`, `/mcp/help`): compile-time `docs/help/*.md` corpus (`build.rs`), front-matter parser, deterministic BM25 search, bounded read/topics/status, RPC. AppHandle-free; `docs/context/help_mcp.md`. |
| `agent_tasks.rs` | Atomic schedule CRUD and claim/complete store; optional agent origin and persistent delivery-budget journal survive retired one-time rows and restarts. |
| `root_mcp_import.rs` | `calendar_import_ics`: stages an agent-sent `.ics` *text* (no path, no URL, never parsed here) under `root_mcp/imports/` for the window's own importer; root callers only, always staged. AppHandle-free. |
| `root_mcp_mail.rs` | The root MCP's nine mail tools (`docs/mail_mcp_plan.md`): `MailAccess` trait (impl: `commands::mail::AgentMail`), whole-result envelope, `strip_invisible`, `redact_urls`, caps, account grants and per-tab draft ownership (`sha256(tab)`, so a resumed tab keeps its drafts) with atomic composer handoff. A root tab's `attach` (via `mail_attach`, limited to the paths `Session::projects_grant` recorded at spawn) and `suggested_to` (stored, never `to`). Read tools answer `Caller::Reader`, and a local-model tab with `root_mcp_mail_local_read` (marked mails, endpoint recorded at spawn must be loopback). AppHandle-free. |
| `mail_reader.rs` | Pure "is this VM narrow right now" check for the contained mail reader (`refusal`, per mail call) and the knob guard (`widen_refusal`, in `vm_set_spec`). |
| `root_mcp_review.rs` | Scoped per-tab calendar copies and quota-bounded proposals; digest-bound grants, atomic preconditions, conflict replay, conditional Undo and generation-safe teardown. AppHandle-free. |
| `root_mcp_security.rs` | Fail-closed policy snapshots, explicit tool/role registry, session scope grants, schema validation, resource budgets and bounded in-memory audit. |
| `agent_fence.rs` | Default-on local agent fence: pure authority/root decisions, box union roots, fence-tool probe (bwrap / `sandbox-exec`), argv wrappers, working-root args; `root_fence_projects_readable` adds every project read-only to a root spawn (`root_project_read_only_paths`, recorded per tab). Remote/container/Windows report not-enforced. Fails closed; final private-state masks outrank project/allowlist mounts. |
| `exec_trust.rs` | Ask-once gate for project-supplied code run on the host (git hooks + exec-capable repo config, `latexmkrc`, a project's own prettier): fingerprint vs `<state_dir>/exec_trust.json`, `eldrun-trust-required:` error, re-asks on change. |
...
| `git_guard.rs` | The `.git` control files (config, config.worktree, hooks, commondir, pointer files) a sandbox re-mounts read-only over the rw project, `.git` pinned onto itself so it can't be swapped for a `gitdir:` pointer (#158). Used by `agent_fence` and `sandbox`. |
...
| `ide_detect.rs` | Which IDE a project tree belongs to (`.idea/` + `.iml` type → JetBrains product, `.vs/`/`*.sln` → Visual Studio, `.vscode/`/`*.code-workspace` → VS Code) and which installed program opens it (`settings.ide_launchers` override → `PATH` → Toolbox scripts → installed apps → `vswhere`), with IDEA / Rider / VS Code stand-ins. Pure over a `HostProbe`; reads marker names only. |
...
| `mail_store.rs` | Encrypted mail persistence; folder search matches only subject/sender like IMAP, while other local lists may search previews. MCP draft changes compare the original row under the database lock, so composer ownership cannot be raced. `staged` rows carry `origin`/`source` (the truth Send reads); `change_draft_files` stages an agent's files under one lock; `outgoing` binds a send to the shown ids. |
| `mail_attach.rs` | A root agent's mail `attach`: same-roots resolution (`agent_fence::attach_roots`, no `/`/`$HOME`/state roots), component checks before I/O, `openat(O_NOFOLLOW)` walk, `O_NONBLOCK` + `fstat` regular, capped read; caps 5 files / 20 MiB / 25 MiB draft / 100 MiB tab. Windows refused. AppHandle-free. |
